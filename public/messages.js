...
(() => {
  ...
  const API_URL = (window.location.hostname === AUTO_HOST || window.location.hostname.endsWith('.onrender.com'))
    ? `https://${AUTO_HOST}/api`
    : '/api';
  ...
  async function doFetch(path, opts = {}) {
    ...
    try {
      ...
      if (!res.ok) throw { status: res.status, body: data, statusText: res.statusText };

      return data;
    } catch (err) {
      // network / CORS errors typically are TypeError
      if (err instanceof TypeError || (err && err.message && err.message.includes('NetworkError'))) {
  ...
  async function loadInbox() {
    ...
    try {
      ...
      messages.forEach(m => messagesList.appendChild(renderMessageCard(m, 'inbox')));
      attachConversationButtons();
    } catch (err) {
      console.error('[loadInbox] error:', err);
      if (err.type === 'network') {
        renderError('Network/CORS error while fetching inbox. See console for details.');
      } else if (err.status === 401) {
        renderError('Unauthorized (401). Please log in again.');
      } else if (err.status === 403) {
        renderError('Forbidden (403). CORS or permission issue. Check server logs and CORS_ALLOWED_ORIGINS.');
      } else {
        const msg = err.body?.error?.message || err.body || err.statusText || err.message || 'Unknown';
  ...
  async function loadSent() {
    ...
    try {
      ...
      messagesList.innerHTML = '';
      messages.forEach(m => messagesList.appendChild(renderMessageCard(m, 'sent')));
      attachConversationButtons();
    } catch (err) {
      console.error('[loadSent] error:', err);
      if (err.type === 'network') {
        renderError('Network/CORS error while fetching sent messages. See console for details.');
      } else if (err.status === 401) {
        renderError('Unauthorized (401). Please log in again.');
      } else if (err.status === 403) {
        renderError('Forbidden (403). CORS or permission issue. Check server logs and CORS_ALLOWED_ORIGINS.');
  ...
  async function loadConversation(otherUserId, otherUsername) {
    ...
    try {
      ...
      if (!conversation.length) {
        messagesList.innerHTML += '<p class="empty">No messages in this conversation yet.</p>';
      } else {
      ...
      if (err.type === 'network') {
        renderError('Network/CORS error while loading conversation. See console for details.');
      } else if (err.status === 401) {
        renderError('Unauthorized (401). Please log in again.');
      } else {
        const msg = err.body?.error?.message || err.body || err.statusText || err.message || 'Unknown';
