...
(function () {
  ...
  const getUserId = safe(window.getUserId, () =>
    localStorage.getItem("userId"),
  );
  const setUserId = safe(window.setUserId, (v) =>
    localStorage.setItem("userId", v),
  );
  const getDisplayName = safe(
    window.getDisplayName,
    (u) => u?.username || u?.name || "User",
  );
  const getDescription = safe(
  ...
  async function doFetch(pathOrUrl, opts = {}, timeoutMs = 8000) {
    ...
    if (Number.isFinite(timeoutMs) && timeoutMs > 0) {
      ...
      if (init.signal) {
        ...
        init.signal.addEventListener("abort", () => chained.abort(), {
          once: true,
        });
  ...
  (async function healthCheck() {
    try {
      ...
      Diag.show(
        'API health check failed. <span class="muted">Verify that <code>/api/health</code> is reachable, check CORS, and confirm the server is running.</span>',
      );
    }
  })();
  ...
  async function loadProfile() {
    try {
      ...
      const data = qsUserId
        ? await doFetch(`users/${profileUserId}`, {}, 8000)
        : { user: me };
