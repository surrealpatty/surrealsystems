...
(async () => {
  try {
    ...
    const rows = await m.sequelize.query('SELECT * FROM ratings LIMIT 5', { type: QueryTypes.SELECT });
    console.log('\nROWS:');
    ...
    const cnt = await m.sequelize.query('SELECT COUNT(*)::int AS cnt FROM ratings', { type: QueryTypes.SELECT });
    console.log('\nCOUNT:');
