...
module.exports = function authenticateToken(req, res, next) {
  try {
    ...
    if (!token)
      return res
        .status(401)
        .json({ success: false, error: { message: "Empty token" } });
    ...
    if (!secrets.length) {
      ...
      return res
        .status(500)
        .json({ success: false, error: { message: "Server misconfigured" } });
    }
    ...
    for (const secret of secrets) {
      try {
        ...
        if (e && e.name === "TokenExpiredError") {
          ...
          return res
            .status(401)
            .json({ success: false, error: { message: "Token expired" } });
        }
    ...
    if (!userId) {
      ...
      return res
        .status(401)
        .json({ success: false, error: { message: "Invalid token payload" } });
    }
