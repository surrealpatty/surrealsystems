...
router.get("/user/:userId", async (req, res) => {
  try {
    ...
    const sum = ratings.reduce((s, r) => {
      ...
      const v =
        get("stars") !== undefined && get("stars") !== null
          ? get("stars")
          : get("score");
      return s + Number(v || 0);
    }, 0);
...
router.post("/", authenticateToken, async (req, res) => {
  try {
    ...
    if (!(await isUserPaid(rater))) {
      return res
        .status(403)
        .json({ error: "Upgrade to a paid account to rate others." });
    }
    ...
    return res
      .status(500)
      .json({ error: "Failed to save rating", details: err.message });
  }
});
