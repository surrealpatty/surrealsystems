...
if (!process.env.JWT_SECRET && process.env.JWT_SECRETS) {
  ...
  if (first) {
    ...
    console.info(
      "Using first secret from JWT_SECRETS as JWT_SECRET (rotation supported).",
    );
  }
}
...
console.info(
  "CORS allowedOrigins:",
  allowedOrigins.length ? allowedOrigins : "[none configured]",
);
...
if (process.env.NODE_ENV !== "production") {
  app.use((req, res, next) => {
    if (req.path && req.path.startsWith("/api/")) {
      console.info(
        `[API] ${req.method} ${req.path} origin=${req.headers.origin || "<none>"}`,
      );
    }
    next();
  });
...
async function startServer() {
  ...
  if (process.env.NODE_ENV !== "production") {
    ...
    if (useAlter) {
      console.log(
        "⚠️ Running sequelize.sync({ alter: true }) - development only",
      );
    } else {
